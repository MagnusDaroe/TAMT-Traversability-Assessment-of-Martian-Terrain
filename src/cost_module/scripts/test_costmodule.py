...
class CostModuleTester(Node):
    ...
    def check_all_received(self):
        ...
        if self.received_segmentation and self.received_normals and self.received_costmap:
    ...
    def generate_results(self):
        ...
        if self.agreement_counts is not None and np.sum(self.agreement_counts > 0) > 0:
            ...
            self.get_logger().info('\n' + '='*60)
            self.get_logger().info('SPATIAL CONSISTENCY ANALYSIS')
            self.get_logger().info('='*60)
            ...
            # Save
            output_path = Path.home() / 'cost_module_test_results.png'
            plt.savefig(output_path, dpi=300, bbox_inches='tight')
            self.get_logger().info(f'\nResults saved to: {output_path}')

            # Save individual plots
            overlap_mask = diff_valid_mask & var_valid_mask
            individual_plot_dir = self.save_individual_plots(
                avg_difference, diff_valid_mask, std_cost, cv_cost, 
                var_valid_mask, mean_cost, overlap_mask
            )
            ...
            data_path = Path.home() / 'cost_module_test_results.npz'
    ...
    def save_individual_plots(self, avg_difference, diff_valid_mask, std_cost, cv_cost, 
                            var_valid_mask, mean_cost, overlap_mask):
        ...
        self.get_logger().info(f'Saving individual plots to: {output_dir}')
        ...
        plt.tight_layout()
        plt.savefig(output_dir / '1_difference_heatmap.png', dpi=dpi, bbox_inches='tight')
        plt.close()
        
        # 2. Difference Sample Counts
        fig, ax = plt.subplots(figsize=(10, 8))
        im = ax.imshow(self.agreement_counts, cmap='viridis', interpolation='nearest', origin='lower')
        ...
        plt.tight_layout()
        plt.savefig(output_dir / '2_difference_sample_counts.png', dpi=dpi, bbox_inches='tight')
        plt.close()
        ...
        if np.any(diff_valid_mask):
            ...
            ax.grid(alpha=0.3)
            plt.tight_layout()
            plt.savefig(output_dir / '3_difference_histogram.png', dpi=dpi, bbox_inches='tight')
            plt.close()
        ...
        plt.tight_layout()
        plt.savefig(output_dir / '4_std_dev_heatmap.png', dpi=dpi, bbox_inches='tight')
        plt.close()
        ...
        plt.tight_layout()
        plt.savefig(output_dir / '5_coefficient_of_variation.png', dpi=dpi, bbox_inches='tight')
        plt.close()
        
        # 6. Variance Sample Counts
        fig, ax = plt.subplots(figsize=(10, 8))
        im = ax.imshow(self.cost_observation_counts, cmap='viridis', 
                    interpolation='nearest', origin='lower')
        ...
        plt.tight_layout()
        plt.savefig(output_dir / '6_variance_sample_counts.png', dpi=dpi, bbox_inches='tight')
        plt.close()
        
        # 7. Segmentation Latency Distribution
        if self.segmentation_latencies:
            fig, ax = plt.subplots(figsize=(10, 8))
            seg_data = np.array(self.segmentation_latencies) * 1000  # Convert to ms
            ...
            ax.grid(alpha=0.3)
            plt.tight_layout()
            plt.savefig(output_dir / '7_segmentation_latency.png', dpi=dpi, bbox_inches='tight')
            plt.close()
        
        # 8. Surface Normals Latency Distribution
        if self.normals_latencies:
            fig, ax = plt.subplots(figsize=(10, 8))
            norm_data = np.array(self.normals_latencies) * 1000  # Convert to ms
            ...
            ax.grid(alpha=0.3)
            plt.tight_layout()
            plt.savefig(output_dir / '8_normals_latency.png', dpi=dpi, bbox_inches='tight')
            plt.close()
        
        # 9. Total Pipeline Latency Distribution
        if self.costmap_latencies:
            fig, ax = plt.subplots(figsize=(10, 8))
            cost_data = np.array(self.costmap_latencies) * 1000  # Convert to ms
            ...
            ax.grid(alpha=0.3)
            plt.tight_layout()
            plt.savefig(output_dir / '9_total_pipeline_latency.png', dpi=dpi, bbox_inches='tight')
            plt.close()

        # 10. Cost Value Distribution Across All Frames
        if np.sum(self.cost_value_histogram) > 0:
            fig, ax = plt.subplots(figsize=(12, 8))
            
            # Normalize by total pixel count to get probability distribution
            total_pixels = np.sum(self.cost_value_histogram[:-1])  # Exclude value 255 (unknown)
            normalized_hist = self.cost_value_histogram[:-1] / total_pixels if total_pixels > 0 else self.cost_value_histogram[:-1]
            ...
            ax.set_title('Normalized Cost Value Distribution', 
                        fontsize=title_fontsize, fontweight='bold')
            ...
            plt.tight_layout()
            plt.savefig(output_dir / '10_cost_value_distribution.png', dpi=dpi, bbox_inches='tight')
            plt.close()
            
            self.get_logger().info('Cost value distribution histogram saved')

        self.get_logger().info(f'Individual plots saved to: {output_dir}')
