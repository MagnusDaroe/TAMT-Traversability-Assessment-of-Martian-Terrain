...
public:
    CostmapSNE() : Node("costmap_sne")
    {
        ...
        RCLCPP_INFO(this->get_logger(), "Loaded camera parameters - FOV X: %.1f°, FOV Y: %.1f°, Max distance: %.2fm, Tilt: %.1f°",
                    fov_x_, fov_y_, max_distance_, tilt_angle_);
        ...
        segmentation_mask_sub_ = this->create_subscription<sensor_msgs::msg::Image>(
            "/segmentation_mask",
            10,
            std::bind(&CostmapSegmentation::segmentationMaskCallback, this, std::placeholders::_1)
        );
...
private:
    ...
    void segmentationMaskCallback(const sensor_msgs::msg::Image::SharedPtr msg)
    {
        ...
        if (!sync_pointcloud_)
        {
            RCLCPP_WARN(this->get_logger(), "No pointcloud received yet, skipping segmentation processing");
            return;
        }
        ...
        RCLCPP_DEBUG(this->get_logger(), "Received segmentation mask: %dx%d, encoding: %s", 
                     width, height, msg->encoding.c_str());
        
        // Step 1: Decode the 16UC1 mask into class IDs and confidence values
        std::vector<uint8_t> class_ids;
        std::vector<float> confidences;
        decodeMask(msg, class_ids, confidences);
        
        // Step 2: Combine pointcloud with segmentation data
        // Output format: [x, y, z, class_id, confidence] for each point
        std::vector<float> points_with_segmentation = combinePointcloudWithSegmentation(
            class_ids, confidences, width, height);
        ...
        // Step 3: Transform points to rover frame (instead of global frame)
        // Output format: [x_rover, y_rover, z_rover, class_id, confidence] for each point
        std::vector<float> points_rover = transformToRoverFrame(points_with_segmentation, width, height);
        RCLCPP_INFO(this->get_logger(), "Transformed points to rover frame");
        
        // Step 4: Compute traversability cost based on segmentation
        // Output format: [x, y, z, cost] for each point
        std::vector<float> points_with_costs = computeTraversabilityCost(points_rover, width, height);
        
        // Step 5: Create averaged cost grid
        ...
        // Step 6: Publish costmap
        publishCostmap(averaged_grid, width_cells, height_cells, origin_x, origin_y);
        
        RCLCPP_INFO(this->get_logger(), "Processed segmentation mask: %dx%d", width, height);
    }
    ...
    {
        // Ensure pointcloud size matches segmentation size
        if (sync_pointcloud_->width != width || sync_pointcloud_->height != height)
        {
            RCLCPP_ERROR(this->get_logger(), 
                         "Size mismatch! Segmentation: %dx%d, Pointcloud: %dx%d",
                         width, height, 
    ...
    std::vector<float> computeTraversabilityCost(const std::vector<float>& points_rover,
                                                  uint32_t width, uint32_t height)
    {
        size_t num_pixels = width * height;
                        ...
                        float origin_x, float origin_y)
    {
        ...
        publishCostmapViz(averaged_grid, width_cells, height_cells, origin_x, origin_y, timestamp);
